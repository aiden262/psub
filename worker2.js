...
var src_default = {
  async fetch(request, env) {
    ...
    const frontendUrl = 'https://raw.githubusercontent.com/aiden262/psub/main/index.html';
    ...
    if (urlParam.startsWith("proxies:")) {
      ...
      for (const url2 of urlParts) {
        ...
        if (/^(ssr?|vmess1?|trojan|vless|tuic|hysteria|hysteria2|tg):\/\//.test(url2)) {
...
function replaceInUri(link, replacements, isRecovery) {
  switch (true) {
    ...
    case link.startsWith("tuic://"):
      return replaceTUIC(link, replacements, isRecovery);
...
function replaceTUIC(link, replacements, isRecovery) {
  ...
  // 修改正则表达式，使password部分更宽松
  const regexMatch = link.match(/tuic:\/\/([\da-f-]+):([^@]+)@([^:]+):/i);
  if (!regexMatch) {
    return;
  }
  ...
  if (isRecovery) {
    return link
      .replace(new RegExp(`${uuid}:${password}@`, "g"), `${replacements[uuid]}:${replacements[password]}@`)
      .replace(new RegExp(`${server}:`, "g"), `${replacements[server]}:`);
  } else {
    return link
      .replace(new RegExp(`${uuid}:${password}@`, "g"), `${randomUUID}:${randomPassword}@`)
      .replace(new RegExp(`${server}:`, "g"), `${randomDomain}:`);
  }
}
